...
ouauth2_scheme = OAuth2PasswordBearer(tokenUrl="users/token")
...
def create_refresh_token(data: dict):
    """
    Создает refresh-токен с длительным сроком действия и token_type=`refresh`
    """
    to_encode = data.copy()
...
async def get_current_user(
    token: str = Depends(ouauth2_scheme), db: AsyncSession = Depends(get_async_db)
):
    """
